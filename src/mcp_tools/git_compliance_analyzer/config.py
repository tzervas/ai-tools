...
class FileExistenceRuleItem(BaseModel):
    ...
    severity: str = Field(
        default="Medium", pattern=r"^(High|Medium|Low|Informational)$"
    )
...
class FileContentRuleItem(BaseModel):
    file_path_pattern: (
        Pattern  # Regex for file paths to check (e.g., "\\.py$", "README\\.md")
    )
    ...
    def compile_file_path_pattern(cls, v):
        if isinstance(v, str):
            try:
                ...
                raise ValueError(
                    f"Invalid regex pattern for file_path_pattern '{v}': {e}"
                )
        return v
...
class CommitHistoryRules(BaseModel):
    ...
    conventional_commit_format: Optional[ConventionalCommitFormatRule] = Field(
        default_factory=ConventionalCommitFormatRule
    )
...
class IaCValidationRuleItem(BaseModel):
    ...
    paths: List[str] = Field(
        default_factory=lambda: ["."]
    )  # Directories to run in, relative to repo root
    severity: str = Field(default="High", pattern=r"^(High|Medium|Low|Informational)$")
    enabled: bool = True
...
class ComplianceRuleConfig(BaseModel):
    ...
    commit_history_checks: Optional[CommitHistoryRules] = Field(
        default_factory=CommitHistoryRules
    )
    iac_validation_checks: Optional[IaCValidationRules] = Field(
        default_factory=IaCValidationRules
    )
    ...
    if not actual_config_path:
        ...
        while True:
            default_path_try = os.path.join(
                current_dir, DEFAULT_COMPLIANCE_RULES_FILENAME
            )
    ...
    if actual_config_path and os.path.exists(actual_config_path):
        ...
        try:
            ...
            if config_data is None:
                print(
                    f"Warning: Compliance rules file '{actual_config_path}' is empty. Using default rules."
                )
                return ComplianceRuleConfig()
            return ComplianceRuleConfig(**config_data)
        except yaml.YAMLError as e:
            raise ValueError(
                f"Error parsing YAML compliance rules file {actual_config_path}: {e}"
            )
        except ValidationError as e:
            raise ValueError(
                f"Compliance rules validation error in {actual_config_path}:\n{e}"
            )
        except Exception as e:
            raise ValueError(
                f"Unexpected error loading compliance rules from {actual_config_path}: {e}"
            )
    else:
        if config_path:
            print(
                f"Warning: Compliance rules file '{config_path}' not found. Using default rules."
            )
        else:
            print(
                f"No compliance rules file '{DEFAULT_COMPLIANCE_RULES_FILENAME}' found. Using default rules."
            )
        return ComplianceRuleConfig()
        ...
        dummy_rules = {
            "file_checks": {
                ...
                "must_not_exist_patterns": [
                    {"pattern": "*.tmp", "severity": "Informational"}
                ],
            },
            "file_content_checks": {
                "rules": [
                    {
                        ...
                        "must_contain_pattern": {
                            "pattern": "License",
                            "message": "MD files should mention License.",
                            "severity": "Low",
                        },
                        "enabled": True,
                    }

...
def run_all_compliance_checks(
    ...
    if base_branch_for_commit_history:
        print(f"Commit history will be compared against: {base_branch_for_commit_history}")

    # 1. File Existence Checks
    if config.file_checks and config.file_checks.enabled:
        print("\nChecking file existence policies...")
        findings = file_checker.check_file_existence(git_utils, target_branch_or_rev, config.file_checks)
        if findings:
    ...
    if config.file_content_checks and config.file_content_checks.enabled:
        ...
        findings = file_checker.check_file_content(git_utils, target_branch_or_rev, config.file_content_checks)
        if findings:
            all_findings.extend(findings)
        else:
    ...
    if config.commit_history_checks and config.commit_history_checks.enabled and base_branch_for_commit_history:
    ...
    elif config.commit_history_checks and config.commit_history_checks.enabled and not base_branch_for_commit_history:
    ...
    if config.iac_validation_checks and config.iac_validation_checks.enabled:
        ...
        findings = iac_checker.check_iac_validations(str(repo_path), config.iac_validation_checks)
        if findings:
            all_findings.extend(findings)
        else:
            print("  IaC validation policies: OK (or no relevant rules triggered errors)")
...
def main():
    ...
    parser.add_argument("--branch", "-b", type=str, default="HEAD",
                        help="The branch, tag, or commit SHA to analyze the state of (default: HEAD).")
    parser.add_argument("--base-branch", type=str, default=None, # e.g. main, develop
    ...
    if not repo_abs_path.is_dir():
        print(f"Error: Repository path '{repo_abs_path}' is not a valid directory.", file=sys.stderr)
        sys.exit(2)

    try:
        git_utils = GitUtils(repo_path=str(repo_abs_path)) # GitUtils handles search_parent_dirs
        ...
        print(f"Error initializing Git repository at '{repo_abs_path}': {e}", file=sys.stderr)
        sys.exit(2)
    ...
    print(f"Loading compliance rules (file: {args.rules_file or 'auto-detect .compliance-rules.yml'})...")
    try:
    ...
    if all_findings:
        ...
        sorted_findings = sorted(
            all_findings,
            key=lambda f: (severity_order.get(f.severity, 99), f.rule_id, f.file_path or "", f.commit_sha or "")
        )
        for i, finding in enumerate(sorted_findings, 1):
            print(f"\nFinding {i}/{len(all_findings)}: {str(finding)}") # Uses ComplianceFinding.__str__
            if finding.details:

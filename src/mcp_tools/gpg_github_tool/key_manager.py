...
def generate_gpg_key(
    gpg_home: str, name: str, email: str, expiry: str, passphrase: str = None
) -> tuple[str, str, str]:
    """
    ...
    if not key_data or not key_data.fingerprint:
        print(
            "Error: GPG key generation failed. No fingerprint returned.",
            file=sys.stderr,
        )
        print(
            f"GPG status: {key_data.status if key_data else 'Unknown'}", file=sys.stderr
        )
    ...
    key_id = fingerprint[
        -16:
    ]  # Typically, the last 16 chars of fingerprint are the short Key ID

    print(
        f"GPG Key generated successfully. Fingerprint: {fingerprint}, Key ID: {key_id}",
        file=sys.stderr,
    )
    ...
    if not public_key_armored:
        print(
            f"Error: Failed to export public key for fingerprint {fingerprint}.",
            file=sys.stderr,
        )
        return None, None, None
    ...
    if not private_key_armored:
        print(
            f"Error: Failed to export private key for fingerprint {fingerprint}.",
            file=sys.stderr,
        )
...
def add_gpg_key_to_github(public_key_armored: str, github_token: str) -> bool:
    ...
    payload = {"armored_public_key": public_key_armored}
    ...
    try:
        ...
        print(
            f"Error adding GPG key to GitHub: {e.response.status_code} - {e.request.url!r}.",
            file=sys.stderr,
        )
        try:
            error_details = e.response.json()
            print(
                f"GitHub Error: {error_details.get('message', 'No message')}",
                file=sys.stderr,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a GPG key and add it to GitHub."
    )
    parser.add_argument(
        "--name",
        required=True,
        help="Real name for the GPG key (e.g., 'Tyler Zervas Agent').",
    )
    parser.add_argument(
        "--email",
        required=True,
        help="Email for the GPG key (e.g., 'tz-dev-agent@vectorwieght.com').",
    )
    parser.add_argument(
        "--expiry",
        default=DEFAULT_KEY_EXPIRY,
        help=f"Expiration for the GPG key (e.g., 0, 1y, 30d). Default: {DEFAULT_KEY_EXPIRY}.",
    )
    parser.add_argument(
        "--passphrase",
        default=None,
        help="Optional passphrase for the GPG key. If not provided, key will have no passphrase (less secure).",
    )
    parser.add_argument(
        "--github-token",
        help="GitHub Personal Access Token with 'write:gpg_key' scope. Can also be set via GITHUB_TOKEN env var.",
    )
    parser.add_argument(
        "--gpg-home",
        default=None,
    ...
    if not github_token:
        print(
            "Error: GitHub token not provided. Set --github-token or GITHUB_TOKEN environment variable.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        "SECURITY WARNING: The following is your GPG private key. Handle it with extreme care.",
        file=sys.stderr,
    )
    print(
        "It is recommended to import it into your primary GPG keyring or agent's environment immediately",
        file=sys.stderr,
    )
    ...
    if args.output_private_key_file:
        try:
            ...
            print(
                f"Private key saved to: {args.output_private_key_file}", file=sys.stderr
            )
            print(
                f"Ensure you protect this file. Consider encrypting it or importing and then deleting it.",
                file=sys.stderr,
            )
        except IOError as e:
            print(
                f"Error writing private key to file {args.output_private_key_file}: {e}",
                file=sys.stderr,
            )
    ...
    print(
        f"1. If you didn't save the private key to a file, copy the private key block above.",
        file=sys.stderr,
    )
    print(
        f"2. Import the private key into your desired GPG environment:", file=sys.stderr
    )
    print(
        f"   gpg --import /path/to/private_key.asc  (or pipe it: ... | gpg --import)",
        file=sys.stderr,
    )
    print(
        f"3. Configure git to use this key ID for signing commits (globally or per repo):",
        file=sys.stderr,
    )
    ...
    print(
        f"4. (Optional) Trust the key if needed: gpg --edit-key {key_id} (then type 'trust', '5', 'y', 'quit')",
        file=sys.stderr,
    )

    if temp_gpg_home_dir:
        print(
            f"Temporary GPG home {gpg_home_path} will be cleaned up.", file=sys.stderr
        )
        temp_gpg_home_dir.cleanup()

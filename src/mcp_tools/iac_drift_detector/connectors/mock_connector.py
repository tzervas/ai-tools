...
class MockActualStateConnector:
        ...
        return [
            {
                ...
                "attributes": {
                    ...
                    "tags": {"Name": "example-instance", "Environment": "prod_actual"} # Tag differs
                }
            },
            {
    ...
    def fetch_actual_state(self, environment_params: Optional[Dict[str, Any]] = None) -> List[ParsedResource]:
        """
        ...
        """
        print(f"MockConnector: Fetching actual state (using {len(self.mock_resources_data)} mock data entries).")
        if environment_params:
            print(f"MockConnector: Received environment params: {environment_params} (currently unused by mock).")
        ...
        for res_data in self.mock_resources_data:
            try:
                ...
                print(f"Warning: Could not parse mock resource data into ParsedResource: {res_data}. Error: {e}", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    if default_state:
        ...
        for res in default_state:
            print(f"  ID: {res.id}, Type: {res.type}, Name: {res.name}, Provider: {res.provider_name}")
            if res.type == "aws_instance":
                assert res.attributes.get("instance_type") == "t2.micro"
                assert res.attributes.get("tags", {}).get("Environment") == "prod_actual"

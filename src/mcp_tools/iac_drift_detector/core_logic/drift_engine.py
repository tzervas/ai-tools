...
DEFAULT_IGNORED_ATTRIBUTES: Dict[str, List[str]] = {
    "aws_instance": ["arn", "private_dns", "public_dns", "public_ip", "ipv6_addresses", "tags_all", "ebs_block_device", "root_block_device", "timeouts"],
    "aws_s3_bucket": ["arn", "bucket_domain_name", "bucket_regional_domain_name", "tags_all", "timeouts"],
...
def compare_attributes(
    ...
    current_ignored_attributes = (ignored_attributes_config or DEFAULT_IGNORED_ATTRIBUTES).get(resource_type, [])
    ...
    for key in all_keys:
        ...
        if key == "tags" and isinstance(iac_value, dict) and isinstance(actual_value, dict):
            ...
            for tag_key, tag_iac_val in iac_value.items():
                ...
                if tag_actual_val != tag_iac_val:
                    drifts.append(AttributeDrift(attribute_name=f"tags.{tag_key}", iac_value=tag_iac_val, actual_value=tag_actual_val))
            ...
            drifts.append(AttributeDrift(attribute_name=key, iac_value=iac_value, actual_value=actual_value))
...
def compare_states(
    ...
    iac_by_id: Dict[str, ParsedResource] = {res.id: res for res in iac_resources if res.id}
    actual_by_id: Dict[str, ParsedResource] = {res.id: res for res in actual_resources if res.id}
    ...
    for iac_res_id, iac_res in iac_by_id.items():
        ...
        if actual_res:
            ...
            if iac_res.type != actual_res.type: # Should ideally not happen if ID is unique across types
                drift_results.append(DriftInfo(
    ...
    actual_ids_processed: Set[str] = set(iac_by_id.keys()) # IDs that were matched or found missing
...
if __name__ == '__main__':
    ...
    from ..connectors.mock_connector import ParsedResource as ActualResource # For clarity

    print("--- Testing Drift Engine ---")

    # Scenario 1: Basic - one modified, one missing, one unmanaged
    iac_state: List[IaCResource] = [
        IaCResource(id="id-instance-01", type="vm", name="web_server_iac", provider_name="mock", attributes={"size": "medium", "image": "ubuntu-20.04", "tags": {"env": "prod"}}),
        IaCResource(id="id-db-01", type="database", name="main_db_iac", provider_name="mock", attributes={"version": "12", "storage": "100GB"}), # This will be missing
    ]
    actual_state: List[ActualResource] = [
        ActualResource(id="id-instance-01", type="vm", name="web_server_actual", provider_name="mock", attributes={"size": "large", "image": "ubuntu-20.04", "tags": {"env": "prod", "extra_tag": "hello"}}), # Modified size
        ActualResource(id="id-disk-unmanaged", type="disk", name="orphan_disk_actual", provider_name="mock", attributes={"size": "50GB"}), # Unmanaged
    ]

    print("\nScenario 1: Modified, Missing, Unmanaged")
    drifts1 = compare_states(iac_state, actual_state)
    for drift in drifts1:
        print(f"  Drift Type: {drift.drift_type.value}, Resource: {drift.resource_type}.{drift.resource_name}, ID: {drift.resource_id}")
        if drift.attribute_drifts:
            for attr_drift in drift.attribute_drifts:
                print(f"    - Attr '{attr_drift.attribute_name}': IaC='{attr_drift.iac_value}', Actual='{attr_drift.actual_value}'")
        if drift.message: print(f"    Msg: {drift.message}")

    assert len(drifts1) == 3
    assert any(d.drift_type == DriftType.MODIFIED and d.resource_id == "id-instance-01" for d in drifts1)
    assert any(d.drift_type == DriftType.MISSING_IN_ACTUAL and d.resource_id == "id-db-01" for d in drifts1)
    assert any(d.drift_type == DriftType.UNMANAGED_IN_ACTUAL and d.resource_id == "id-disk-unmanaged" for d in drifts1)


    # Scenario 2: No drift
    iac_state_no_drift: List[IaCResource] = [
        IaCResource(id="id-vm-100", type="vm", name="app_server", provider_name="mock", attributes={"image": "centos8", "cpu": 2}),
    ]
    actual_state_no_drift: List[ActualResource] = [
        ActualResource(id="id-vm-100", type="vm", name="app_server_live", provider_name="mock", attributes={"image": "centos8", "cpu": 2}),
    ]
    print("\nScenario 2: No Drift")
    ...
    if not drifts2:
        ...
        for drift in drifts2: print(f"  Unexpected Drift: {drift.drift_type.value} for {drift.resource_name}")
    assert not drifts2
    ...
    iac_state_ignore: List[IaCResource] = [
        IaCResource(id="id-vm-200", type="vm", name="worker", provider_name="mock",
                    attributes={"image": "debian", "ram": "4GB", "last_updated_time": "ts1", "tags": {"managed_by": "iac"}}),
    ]
    actual_state_ignore: List[ActualResource] = [
        ActualResource(id="id-vm-200", type="vm", name="worker_live", provider_name="mock",
    ...
    drifts3 = compare_states(iac_state_ignore, actual_state_ignore, ignored_attributes_config=custom_ignored)
    if not drifts3: # Expect no drift due to ignore rules for attributes and tags handling (extra actual tags are ignored)
        print("  No drift detected (attributes ignored, extra actual tags ignored). OK.")
    else:
        for drift in drifts3:
            print(f"  Unexpected Drift: {drift.drift_type.value} for {drift.resource_name}")
            if drift.attribute_drifts:
                for ad in drift.attribute_drifts: print(f"    Attr: {ad.attribute_name}")
    ...
    iac_state_tag_mod: List[IaCResource] = [
        IaCResource(id="id-vm-300", type="vm", name="tagged_vm", provider_name="mock",
                    attributes={"tags": {"env": "staging", "owner": "team-a"}}),
    ]
    actual_state_tag_mod: List[ActualResource] = [
        ActualResource(id="id-vm-300", type="vm", name="tagged_vm_live", provider_name="mock",
                       attributes={"tags": {"env": "prod", "owner": "team-a", "new_tag": "val"}}), # env tag changed
    ]

...
from .parsers.terraform_parser import parse_terraform_state_file, parse_terraform_plan_json_file, ParsedResource as IaCResource
...
def main():
    ...
    actual_state_group.add_argument("--actual-state-source", type=str, default="mock", choices=["mock"], # Add "aws", "gcp" later
    ...
    if args.iac_type == "terraform":
        if args.tf_state_file:
            print(f"Loading desired state from Terraform state file: {args.tf_state_file}")
        ...
        # elif args.tf_plan_file:
            ...
            print("Error: For Terraform, either --tf-state-file must be provided.", file=sys.stderr)
            sys.exit(2)
    else:
        print(f"Error: IaC type '{args.iac_type}' is not yet supported.", file=sys.stderr)
        sys.exit(2)

    if not desired_state_resources and args.tf_state_file and os.path.exists(args.tf_state_file):
    ...
    # 2. Load Actual State
    actual_state_resources: List[IaCResource] = [] # Using IaCResource type for consistency from models.py
    ...
    #     sys.exit(3)
        ...
        print(f"Error: Actual state source '{args.actual_state_source}' is not yet supported.", file=sys.stderr)
        sys.exit(3)
    ...
    for i, drift_info in enumerate(detected_drifts, 1):
        ...
        if drift_info.message and drift_info.drift_type != DriftType.MODIFIED : # MODIFIED has detailed attr reporting
        ...
        if drift_info.drift_type == DriftType.MODIFIED and drift_info.attribute_drifts:
            ...
            for attr_drift in drift_info.attribute_drifts:
                ...
                act_val_str = f"'{attr_drift.actual_value}'" if attr_drift.actual_value is not None else "not set (None)"
                print(f"    - '{attr_drift.attribute_name}': IaC = {iac_val_str}, Actual = {act_val_str}")

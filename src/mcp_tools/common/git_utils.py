...
class GitUtils:
    ...
    def list_files_at_revision(self, revision: str = 'HEAD', file_glob_patterns: Optional[List[str]] = None) -> List[str]:
        """
        ...
        try:
            ...
            for filepath_str in all_filepaths:
                ...
                if any(Path(filepath_str).match(pattern) for pattern in file_glob_patterns):
            ...
            raise GitRepoError(f"Invalid revision or other error in list_files_at_revision ('{revision}'): {e}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if current_branch and current_branch != "main" and current_branch != "master":
            ...
            python_files = utils.list_files_at_revision('HEAD', file_glob_patterns=['*.py', '**/*.py']) # Test glob
            if python_files:

...
import os
from pathlib import Path  # Added for Path.match()
...
class GitUtils:
    def __init__(self, repo_path: Optional[str] = None):
        ...
        try:
            ...
            raise GitRepoError(
                f"Error initializing Git repository at {self.repo_path}: {e}"
            )
    ...
    def get_commits_between(self, base_rev: str, head_rev: str) -> List[git.Commit]:
        ...
        try:
            ...
            raise GitRepoError(
                f"Error getting commits between '{base_rev}' and '{head_rev}': {e}"
            )
        except Exception as e:  # Catch other potential errors like bad revision names
            raise GitRepoError(
                f"Invalid revision or error in get_commits_between ('{base_rev}', '{head_rev}'): {e}"
            )
    ...
    def get_commit_details(self, commit: git.Commit) -> dict:
        ...
        return {
            ...
            "message_body": commit.message.replace(
                commit.summary, "", 1
            ).lstrip(),  # Rest of the message
    ...
    def get_changed_files_in_commit(self, commit_sha: str) -> List[str]:
        ...
        try:
            ...
            raise GitRepoError(
                f"Error getting changed files for commit '{commit_sha}': {e}"
            )
        except Exception as e:
            raise GitRepoError(
                f"Invalid commit SHA or error in get_changed_files_in_commit ('{commit_sha}'): {e}"
            )
    ...
    def get_all_changed_files_in_range(self, base_rev: str, head_rev: str) -> Set[str]:
        ...
        for commit in commits:
            ...
            parent_tree = (
                commit.parents[0].tree if commit.parents else self.repo.tree()
            ...
            for diff_item in diffs:
                ...
                if (
                    diff_item.b_path
                ):  # File exists in the "after" state (added, modified, renamed to)
                    all_files.add(diff_item.b_path)
                elif (
                    diff_item.a_path
                ):  # File existed in "before" state and was deleted/renamed from
                    all_files.add(
                        diff_item.a_path
                    )  # Track deleted files too for some checks
        return all_files

    def get_file_content_at_revision(
        self, filepath: str, revision: str = "HEAD"
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            raise GitRepoError(
                f"Error getting content for file '{filepath}' at revision '{revision}': {e}"
            )

    def get_file_size_at_revision(
        self, filepath: str, revision: str = "HEAD"
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            raise GitRepoError(
                f"Error getting size for file '{filepath}' at revision '{revision}': {e}"
            )

    def list_files_at_revision(
        self, revision: str = "HEAD", file_glob_patterns: Optional[List[str]] = None
    ) -> List[str]:
        """
        Lists all files in the repository at a specific revision, optionally filtered by glob patterns.

        Args:
            revision: The revision to list files from (e.g., 'HEAD', branch name, commit SHA).
            file_glob_patterns: An optional list of glob patterns to filter files.
                                If None or empty, all files are listed.
                                Uses Path.match() for globbing.

        Returns:
            A list of file paths relative to the repository root.

        Raises:
            GitRepoError: If the revision is invalid or other Git errors occur.
        """
        try:
            commit = self.repo.commit(revision)
            tree = commit.tree
            ...
            for blob in tree.traverse():  # traverse() explores the tree recursively
                if blob.type == "blob":  # Ensure it's a file, not a tree (directory)
                    all_filepaths.append(blob.path)

            if not file_glob_patterns:
                return sorted(all_filepaths)

            filtered_filepaths: List[str] = []
            for filepath_str in all_filepaths:
                # Path.match() requires the pattern to match the whole string,
                # and works well with relative paths from the root of the match.
                # Here, blob.path is already relative to repo root.
                if any(
                    Path(filepath_str).match(pattern) for pattern in file_glob_patterns
                ):
                    filtered_filepaths.append(filepath_str)

            return sorted(filtered_filepaths)

        except git.GitCommandError as e:
            raise GitRepoError(f"Error listing files at revision '{revision}': {e}")
        except Exception as e:
            raise GitRepoError(
                f"Invalid revision or other error in list_files_at_revision ('{revision}'): {e}"
            )
    ...
    try:
        ...
        if current_branch and current_branch != "main" and current_branch != "master":
            ...
            if potential_bases:
                ...
                commits = utils.get_commits_between(
                    base_branch_for_test, "HEAD"
                )  # 'HEAD' or current_branch
                if commits:
                    for i, commit in enumerate(commits):
                        details = utils.get_commit_details(commit)
                        print(
                            f"  Commit {i+1}: {details['sha'][:7]} - {details['message_subject']}"
                        )
                    ...
                    all_changed = utils.get_all_changed_files_in_range(
                        base_branch_for_test, "HEAD"
                    )
                    ...
                    print(
                        f"No commits found between '{base_branch_for_test}' and '{current_branch}'."
                    )
            else:
                print(
                    "Could not determine a base branch (like main/develop) for testing commit ranges."
                )

        else:
            print(
                "Skipping commit range tests (current branch is main/master or detached)."
            )
            ...
            print("\n--- List all Python files at HEAD ---")
            python_files = utils.list_files_at_revision(
                "HEAD", file_glob_patterns=["*.py", "**/*.py"]
            )  # Test glob
            if python_files:
                print(f"Found {len(python_files)} Python files (showing first 5):")
                for py_file in python_files[:5]:
                    print(f"  - {py_file}")
            else:
                print("No Python files found with specified patterns.")

            print("\n--- List all files at HEAD (no filter) ---")
            all_files_head = utils.list_files_at_revision("HEAD")
            print(f"Total files at HEAD: {len(all_files_head)} (showing first 5):")
            for f_path in all_files_head[:5]:
                print(f"  - {f_path}")
        ...
        # Add from pathlib import Path at the top of the file if not already there for Path().match()

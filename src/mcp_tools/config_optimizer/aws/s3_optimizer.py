...
def check_s3_bucket_optimizations(
    ...
    if rules.versioning.enabled:
        ...
        if isinstance(versioning_config, list) and versioning_config: # TF state style
            ...
            if isinstance(versioning_dict, dict) and versioning_dict.get("enabled") is True:
                versioning_status_enabled = True
        elif isinstance(versioning_config, dict): # More direct API style or simplified state
            if versioning_config.get("enabled") is True or versioning_config.get("status") == "Enabled":
                versioning_status_enabled = True
    ...
    if rules.public_access_block.enabled:
        ...
        if isinstance(pab_config, list) and pab_config:
            pab_settings_to_check = pab_config[0] if isinstance(pab_config[0], dict) else {}
        ...
        if rules.public_access_block.require_all_blocks_true:
            ...
            all_blocks_true = all(pab_settings_to_check.get(block_name) is True for block_name in expected_true_blocks)
...
if __name__ == '__main__':
    ...
    s3_no_sse = ParsedResource(
        id="bucket-no-sse", type="aws_s3_bucket", name="no-sse-bucket", provider_name="aws",
        attributes={"bucket": "bucket-no-sse", "acl": "private"}
    )
    s3_with_aes256 = ParsedResource(
        id="bucket-aes256", type="aws_s3_bucket", name="aes256-bucket", provider_name="aws",
        attributes={
            "bucket": "bucket-aes256", "acl": "private",
            "server_side_encryption_configuration": {
    ...
    s3_with_kms = ParsedResource(
        id="bucket-kms", type="aws_s3_bucket", name="kms-bucket", provider_name="aws",
        attributes={
            "bucket": "bucket-kms", "acl": "private",
            "server_side_encryption_configuration": {
                "rule": {"apply_server_side_encryption_by_default": {
                    "sse_algorithm": "aws:kms", "kms_master_key_id": "alias/aws/s3"
                }}
            }
        }
    )
    s3_no_versioning = ParsedResource(
        id="bucket-no-versioning", type="aws_s3_bucket", name="no-versioning", provider_name="aws",
        attributes={"bucket": "no-versioning"}
    )
    s3_versioning_enabled_tf_style = ParsedResource( # Terraform state list style
        id="bucket-versioned-tf", type="aws_s3_bucket", name="versioned-tf", provider_name="aws",
        attributes={"bucket": "versioned-tf", "versioning": [{"enabled": True, "mfa_delete": False}]}
    )
    s3_pab_incomplete = ParsedResource(
        id="bucket-pab-bad", type="aws_s3_bucket", name="pab-bad", provider_name="aws",
        attributes={"bucket": "pab-bad", "public_access_block": { # Direct dict style
    ...
    s3_pab_good_tf_style = ParsedResource( # TF state list style
        id="bucket-pab-good", type="aws_s3_bucket", name="pab-good", provider_name="aws",
        attributes={"bucket": "pab-good", "public_access_block": [{
    ...
    assert any(r.rule_id == "AWS_S3_PUBLIC_ACCESS_BLOCK_INCOMPLETE" for r in recs_no_sse)


    recs_aes256 = check_s3_bucket_optimizations(s3_with_aes256, default_s3_rules)
    print(f"Recs for s3_with_aes256 (default require_sse_kms=False): {len(recs_aes256)}") # No SSE violation
    assert not any(r.rule_id == "AWS_S3_ENCRYPTION_DISABLED" for r in recs_aes256)
    assert any(r.rule_id == "AWS_S3_VERSIONING_DISABLED" for r in recs_aes256) # Still needs versioning & PAB

    # Test require_sse_kms = True
    custom_s3_rules_kms = AWSS3Rules()
    custom_s3_rules_kms.encryption.require_sse_kms = True
    recs_aes256_kms_req = check_s3_bucket_optimizations(s3_with_aes256, custom_s3_rules_kms)
    print(f"Recs for s3_with_aes256 (require_sse_kms=True): {len(recs_aes256_kms_req)}") # SSE violation now
    ...
    print(f"Recs for s3_with_kms (require_sse_kms=True): {len(recs_kms)}") # No SSE violation
    assert not any(r.rule_id == "AWS_S3_ENCRYPTION_DISABLED" for r in recs_kms)

    recs_versioned_tf = check_s3_bucket_optimizations(s3_versioning_enabled_tf_style, default_s3_rules)
    ...
    recs_pab_incomplete = check_s3_bucket_optimizations(s3_pab_incomplete, default_s3_rules)
    print(f"Recs for s3_pab_incomplete: {len(recs_pab_incomplete)}")
    assert any(r.rule_id == "AWS_S3_PUBLIC_ACCESS_BLOCK_INCOMPLETE" for r in recs_pab_incomplete)
    assert "block_public_policy" in recs_pab_incomplete[-1].details.get("blocks_not_fully_enabled", {}) # Assuming PAB is last check

    recs_pab_good = check_s3_bucket_optimizations(s3_pab_good_tf_style, default_s3_rules)
    print(f"Recs for s3_pab_good_tf_style: {len(recs_pab_good)}")
    assert not any(r.rule_id == "AWS_S3_PUBLIC_ACCESS_BLOCK_INCOMPLETE" for r in recs_pab_good)

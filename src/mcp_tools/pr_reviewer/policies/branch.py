...
from ..config import BranchNamingPolicy # Using relative import for sibling module in package
...
def check_branch_name_policy(
    ...
    if not branch_name:
        violations.append("Branch name could not be determined (possibly detached HEAD). Cannot check naming policy.")
        return violations
    ...
    if policy.pattern:
        ...
        violations.append("Branch naming policy is enabled, but no pattern is configured.")
...
if __name__ == '__main__':
    ...
    print(f"Test 1 (feature/new-login vs {policy1.pattern.pattern}): Violations: {violations1}") # Expected: []

    # Test case 2: Invalid branch name
    violations2 = check_branch_name_policy("Feature/NewLogin", policy1)
    print(f"Test 2 (Feature/NewLogin vs {policy1.pattern.pattern}): Violations: {violations2}") # Expected: [violation message]
    ...
    print(f"Test 4 (detached HEAD): Violations: {violations4}") # Expected: [violation message]
    ...
    print(f"Default pattern: {default_policy.pattern.pattern if default_policy.pattern else 'None'}")

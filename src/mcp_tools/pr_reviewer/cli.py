...
from ..common.git_utils import GitUtils, GitRepoError # Updated import path
...
def run_all_checks(config: PolicyConfig, git_utils: GitUtils, base_branch: str, head_branch: str) -> List[str]:
    """
    ...
    if config.branch_naming.enabled:
        ...
        current_branch_name = git_utils.get_current_branch_name() # Assumes head_branch is the current one
        ...
        if head_branch.upper() == 'HEAD': # get_current_branch_name handles detached HEAD
            branch_to_check = current_branch_name

        branch_violations = branch_policies.check_branch_name_policy(branch_to_check, config.branch_naming)
        if branch_violations:
            all_violations.extend(branch_violations)
            for v in branch_violations: print(f"  Violation: {v}")
        else:
    ...
    try:
        ...
        if not commits_to_check:
            ...
            all_changed_filepaths_in_range = git_utils.get_all_changed_files_in_range(base_branch, head_branch)
            print(f"Found {len(all_changed_filepaths_in_range)} unique changed files in this range.")
    ...
    if (config.disallowed_patterns.enabled or config.file_size.enabled) and all_changed_filepaths_in_range:
        ...
        for filepath in sorted(list(all_changed_filepaths_in_range)):
            ...
            if file_specific_violations:
                ...
                for v_msg in file_specific_violations: print(f"  Violation: {v_msg}")
...
def main():
    ...
    try:
        ...
        print(f"Loading configuration (file: {args.config_file or 'auto-detect .pr-policy.yml'})...")

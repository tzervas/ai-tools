...
def test_cli_commit_message_missing_issue_violation(temp_git_repo: Path):
    ...
    result = run_cli(temp_git_repo, ["--base-branch", "main", "--head-branch", "feature/test-issue"])
    # print("STDOUT:", result.stdout)
    # print("STDERR:", result.stderr)
    assert result.returncode == 1
    assert "No issue number matching pattern 'TASK-\\d+' found in commit message body" in result.stdout
...
def test_cli_disallowed_pattern_violation(temp_git_repo: Path):
    ...
    (temp_git_repo / "secret_file.txt").write_text("This file contains DO_NOT_COMMIT string.")
    repo.index.add(["secret_file.txt"])
    repo.index.commit("feat: add secret file")

    result = run_cli(temp_git_repo, ["--base-branch", "main", "--head-branch", "feature/disallowed"])
    # print("STDOUT:", result.stdout)
    # print("STDERR:", result.stderr)
    assert result.returncode == 1
    assert "File 'secret_file.txt', line 1: Found forbidden string (Matched: 'DO_NOT_COMMIT')" in result.stdout
...
def test_cli_file_size_violation(temp_git_repo: Path):
    ...
    result = run_cli(temp_git_repo, ["--base-branch", "main", "--head-branch", "feature/large-file"])
    # print("STDOUT:", result.stdout)
    # print("STDERR:", result.stderr)
    assert result.returncode == 1
    assert "File 'large_file.txt' (size: 200 bytes) exceeds maximum allowed size of 100 bytes" in result.stdout
...
def test_cli_multiple_violations(temp_git_repo: Path):
    ...
    (temp_git_repo / "big.txt").write_text("This content is definitely more than fifty bytes long for sure.")
    repo.index.add(["big.txt"])
    repo.index.commit("docs: add big file and wrong commit type") # 'docs' type violation

    result = run_cli(temp_git_repo, ["--base-branch", "main", "--head-branch", "fix/wrong-type-branch-for-policy"])
    # print("STDOUT:", result.stdout)
    # print("STDERR:", result.stderr)
    assert result.returncode == 1
    assert "Branch name 'fix/wrong-type-branch-for-policy' does not match" in result.stdout
    assert "Type 'docs' is not one of the allowed types: feat" in result.stdout
    assert "File 'big.txt' (size: 68 bytes) exceeds maximum allowed size of 50 bytes" in result.stdout

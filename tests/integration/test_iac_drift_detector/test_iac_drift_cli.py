...
def run_iac_cli(repo_path: Path, args: list[str]) -> subprocess.CompletedProcess:
    ...
    )  # Run from tmp_path itself or a sub-repo if needed
...
def test_cli_no_tfstate_file_error(tmp_path: Path):
    ...
    result_not_found = run_iac_cli(
        tmp_path, ["--iac-type", "terraform", "--tf-state-file", "nonexistent.tfstate"]
    )
    # print("STDOUT:", result_not_found.stdout)
    # print("STDERR:", result_not_found.stderr)
    assert result_not_found.returncode != 0  # Should be 2
    assert (
        "Error: Terraform state file nonexistent.tfstate not found."
        in result_not_found.stdout
    )  # Printed to stdout then error to stderr


def test_cli_drift_detection_with_mock_connector(
    temp_tfstate_file: Path, tmp_path: Path
):
    """
    ...
    args = [
        ...
        "terraform",
        "--tf-state-file",
        tfstate_file_abs_path,
        "--actual-state-source",
        "mock",
    ]
    result = run_iac_cli(
        tmp_path, args
    )  # Run from tmp_path which is parent of tfstate_file
    ...
    assert (
        "3 DRIFT(S) DETECTED" in result.stdout
    )  # instance modified, db missing, s3 modified + 1 unmanaged volume = 4 drifts
    ...
    assert (
        "'tags.Environment': IaC = not set (None), Actual = 'prod_actual'"
        in result.stdout
    )  # Drift in tags

    assert (
        "Drift Type: MISSING_IN_ACTUAL" in result.stdout
    )  # Check one of the other types
    ...
    assert (
        "'acl': IaC = 'private', Actual = 'public-read'" in result.stdout
    )  # S3 ACL drift
    ...
    assert (
        "terraform import aws_ebs_volume.unmanaged-data-volume vol-09876fedcba"
        in result.stdout
    )
...
def test_cli_no_drift_scenario(tmp_path: Path):
    ...
    args = [
        ...
        "terraform",
        "--tf-state-file",
        str(tfstate_file_v2.resolve()),
        "--actual-state-source",
        "mock",  # Uses default mock
    ]

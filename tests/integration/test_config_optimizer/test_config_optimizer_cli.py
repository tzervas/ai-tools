...
SAMPLE_TFSTATE_CONTENT = {
    ...
    "resources": [
        ...
        { # EC2 - Large type, should be flagged by default rules
            ...
            "provider": "provider[\"registry.terraform.io/hashicorp/aws\"]",
            "instances": [{"attributes": {"id": "i-m5huge", "instance_type": "m5.24xlarge"}}]
        },
        { # S3 - No encryption, no versioning, no PAB
            "mode": "managed", "type": "aws_s3_bucket", "name": "insecure_bucket",
            "provider": "provider[\"registry.terraform.io/hashicorp/aws\"]",
            "instances": [{"attributes": {"id": "bucket-insecure", "bucket": "bucket-insecure"}}]
        },
        { # S3 - Good config (matching default optimizer expectations for "no recs")
            "mode": "managed", "type": "aws_s3_bucket", "name": "secure_bucket",
            "provider": "provider[\"registry.terraform.io/hashicorp/aws\"]",
            "instances": [{"attributes": {
                "id": "bucket-secure", "bucket": "bucket-secure",
...
def test_cli_optimizer_default_rules_produce_recommendations(temp_tfstate_file_optimizer: Path, tmp_path: Path):
    ...
    assert not ("secure_bucket" in result.stdout and "AWS_S3_ENCRYPTION_DISABLED" in result.stdout)


def test_cli_optimizer_custom_rules_file(temp_tfstate_file_optimizer: Path, temp_optimizer_rules_file_custom, tmp_path: Path):
    custom_rules = {
        "aws_ec2": {
    ...
    assert not ("AWS_EC2_NEWER_GENERATION_MAPPED" in result.stdout and "t2.large" in result.stdout)
    ...
    assert not ("AWS_EC2_LARGE_INSTANCE_TYPE" in result.stdout and "m5.24xlarge" in result.stdout)
...
def test_cli_optimizer_no_recommendations(tmp_path: Path, temp_optimizer_rules_file_custom):

...
def temp_git_repo_for_compliance(tmp_path: Path):
    ...
    (repo_dir / "README.md").write_text(
        "Initial README for main branch.\n## Usage\nDetails here."
    )
...
def run_compliance_cli(cwd_path: Path, args: list[str]) -> subprocess.CompletedProcess:
    ...
    project_root = Path(
        __file__
    ).parent.parent.parent.parent  # Adjust if test file moves
    return subprocess.run(cmd, capture_output=True, text=True, cwd=project_root)


def create_compliance_rules_file(
    repo_path: Path,
    rules_content: dict,
    filename: str = DEFAULT_COMPLIANCE_RULES_FILENAME,
):
...
def test_cli_compliance_no_violations_default_rules(
    temp_git_repo_for_compliance: Path, tmp_path: Path
):
    ...
    if (
        "Initial commit: Add README" in result.stdout
    ):  # Check if it processed this commit
...
def test_cli_compliance_with_violations(
    temp_git_repo_for_compliance: Path, tmp_path: Path
):
    repo_abs_path = str(temp_git_repo_for_compliance.resolve())
    rules = {
            ...
            "rules": [
                {
                    ...
                    "must_contain_pattern": {
                        ...
                        "message": "README needs Installation section.",
                        "severity": "Low",
                    },
                }
            ]
        ...
        "iac_validation_checks": {  # This will fail if terraform not installed or no .tf files
            "rules": [
                {"type": "terraform_validate", "paths": ["."], "severity": "High"}
            ]
        },
    }
    ...
    (temp_git_repo_for_compliance / "feature.py").write_text(
        "print('hello')\n# TODO an unlinked todo"
    )
    (temp_git_repo_for_compliance / "app.log").write_text(
        "some log data"
    )  # Violates must_not_exist_patterns
    repo.index.add(["feature.py", "app.log"])
    repo.index.commit(
        "WIP: adding feature without proper type"
    ...
    rules["file_content_checks"]["rules"].append(
        {
            ...
            "must_not_contain_pattern": {
                "pattern": "TODO an unlinked todo",
                "message": "Unlinked TODO found.",
                "severity": "Low",
            },
            "enabled": True,
        }
    )
    create_compliance_rules_file(
        temp_git_repo_for_compliance, rules
    )  # Re-create with added rule
    ...
    args = [
        ...
        "feature/bad-commit",
        "--base-branch",
        "develop",  # Compare against develop for commit history
        "--rules-file",
        str(
            (temp_git_repo_for_compliance / DEFAULT_COMPLIANCE_RULES_FILENAME).resolve()
        ),
    ]
    result = run_compliance_cli(tmp_path, args)
    ...
    assert (
        "FILE_MUST_NOT_EXIST_PRESENT" in output
        and "app.log" in output
        and "*.log" in output
    )
    assert (
        "FILE_CONTENT_MUST_CONTAIN_MISSING" in output
        and "README.md" in output
        and "## Installation" in output
    )
    assert (
        "FILE_CONTENT_MUST_NOT_CONTAIN_PRESENT" in output
        and "feature.py" in output
        and "Unlinked TODO found" in output
    )
    assert (
        "COMMIT_CONVENTIONAL_FORMAT_INVALID" in output
        and "WIP: adding feature" in output
    )
    ...
    assert (
        f"{num_expected_violations} FINDING(S)" in output
        or f"{num_expected_violations+1 if 'TERRAFORM_VALIDATE_CMD_NOT_FOUND' not in output else num_expected_violations} FINDING(S)"
        in output
    )


def test_cli_compliance_rules_file_not_found(
    temp_git_repo_for_compliance: Path, tmp_path: Path, capsys
):
    repo_abs_path = str(temp_git_repo_for_compliance.resolve())
    ...
    result = run_compliance_cli(
        tmp_path, args
    )  # Should use default rules and print a warning
    ...
    assert (
        "Warning: Compliance rules file 'non_existent_rules.yml' not found. Using default rules."
        in result.stdout
    )
    ...
    assert (
        result.returncode == 1
    )  # Because default rules will find the initial commit non-compliant
    assert "COMMIT_CONVENTIONAL_FORMAT_INVALID" in result.stdout

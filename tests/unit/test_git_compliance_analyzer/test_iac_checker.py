...
from src.mcp_tools.git_compliance_analyzer.config import IaCValidationRules, IaCValidationRuleItem
...
def test_run_iac_validation_command_terraform_validate_success(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    ...
    rule = IaCValidationRuleItem(type="terraform_validate", paths=["infra"], severity="High", enabled=True)
...
def test_run_iac_validation_command_terraform_validate_failure(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    mock_process = MagicMock()
...
def test_run_iac_validation_command_cmd_not_found(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    mock_subprocess_run.side_effect = FileNotFoundError("terraform command not found")

    rule = IaCValidationRuleItem(type="terraform_validate", paths=["infra"], enabled=True)
    findings = iac_checker.run_iac_validation_command(temp_repo_path, rule)
...
def test_run_iac_validation_command_path_not_dir(temp_repo_path: Path):
    rule = IaCValidationRuleItem(type="terraform_validate", paths=["infra/main.tf"], enabled=True) # Path is a file
    findings = iac_checker.run_iac_validation_command(temp_repo_path, rule)
    assert len(findings) == 1
...
def test_run_iac_validation_command_path_outside_repo(temp_repo_path: Path):
    ...
    rule = IaCValidationRuleItem(type="terraform_validate", paths=["../outside_repo_sim"], enabled=True)
    findings = iac_checker.run_iac_validation_command(temp_repo_path, rule)
    assert len(findings) == 1
    assert findings[0].rule_id == "IAC_VALIDATION_PATH_OUTSIDE_REPO"

def test_run_iac_validation_command_rule_disabled(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    rule = IaCValidationRuleItem(type="terraform_validate", paths=["infra"], enabled=False)
    findings = iac_checker.run_iac_validation_command(temp_repo_path, rule)
    assert not findings
...
def test_check_iac_validations_multiple_rules(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    ...
    rules_config = IaCValidationRules(rules=[
        IaCValidationRuleItem(type="terraform_validate", paths=["infra"], severity="High", enabled=True),
        IaCValidationRuleItem(type="terraform_validate", paths=["modules/module_a"], severity="Medium", enabled=True)
    ], enabled=True)
...
def test_check_iac_validations_parent_rules_disabled(mock_subprocess_run: MagicMock, temp_repo_path: Path):
    rules_config = IaCValidationRules(rules=[
        IaCValidationRuleItem(type="terraform_validate", paths=["infra"], enabled=True)
    ], enabled=False) # Parent group disabled
...
def test_check_iac_validations_invalid_repo_path():
    rules_config = IaCValidationRules(rules=[
        IaCValidationRuleItem(type="terraform_validate", paths=["."])
    ], enabled=True)
    findings = iac_checker.check_iac_validations("/path/to/non_existent_repo_for_iac_check", rules_config)
    assert len(findings) == 1

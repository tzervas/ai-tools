...
from src.mcp_tools.git_compliance_analyzer.config import (
    ...
    DEFAULT_COMPLIANCE_RULES_FILENAME,
)
...
def test_default_compliance_rule_config():
    ...
    assert isinstance(
        config.commit_history_checks, CommitHistoryRules
    )  # Optional, but default_factory creates it
    assert config.commit_history_checks.enabled is True
    assert isinstance(
        config.commit_history_checks.conventional_commit_format,
        ConventionalCommitFormatRule,
    )
    assert config.commit_history_checks.conventional_commit_format.enabled is True

    assert isinstance(
        config.iac_validation_checks, IaCValidationRules
    )  # Optional, but default_factory
...
def test_content_pattern_rule_regex_compilation():
    ...
    with pytest.raises(
        ValidationError
    ):  # Pydantic v2 raises ValidationError for validator issues
        ContentPatternRule(pattern="[invalid(", message="Test", severity="Low")


def test_file_content_rule_item_regex_compilation():
    item = FileContentRuleItem(
        file_path_pattern=".*\\.py$", enabled=True
    )  # No content patterns needed for this test
...
def test_load_compliance_rules_no_file(tmp_path: Path):
    ...
    try:
        ...
        assert config.file_checks.must_exist == []
        assert (
            config.commit_history_checks.conventional_commit_format.types is not None
        )  # Check a default list
    finally:
...
def test_load_compliance_rules_from_file(temp_compliance_rules_file):
    rules_data = {
        "file_checks": {
            ...
            "must_not_exist_patterns": [
                {"pattern": "*.exe", "message": "Executables not allowed"}
            ],
        },
        "file_content_checks": {
            "rules": [
        ...
        "iac_validation_checks": {
            "rules": [
                {"type": "terraform_validate", "paths": ["./infra"], "severity": "High"}
            ],
            "enabled": False,
        },
    }
    ...
    try:
        config = (
            load_compliance_rules()
        )  # Auto-detect DEFAULT_COMPLIANCE_RULES_FILENAME
    finally:
        os.chdir(original_cwd)
...
def test_load_compliance_rules_pydantic_validation_error(temp_compliance_rules_file):
    invalid_data = {
        "file_checks": {"must_exist": [{"path": 123}]}
    }  # path should be str
...
def test_load_rules_explicit_path_not_found(capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "Warning: Compliance rules file 'non_existent_rules.yml' not found."
        in captured.out
    )

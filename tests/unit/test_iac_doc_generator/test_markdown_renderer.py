...
from src.mcp_tools.iac_doc_generator.markdown_renderer import (
    MarkdownRenderer,
    format_value,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"a": 1, "b": 2, "longkeylongkeylongkeylongkeylongkey": 3},
            "`{'a': 1, 'b': 2, 'longkeylongkeylongkeylongkeylo...`",
        ),  # Truncated dict
        (1.23, "`1.23`"),
    ],
)
...
def sample_module_doc_data() -> TerraformModuleProcessedDoc:
    ...
    var2 = TerraformVariableDoc(
        name="image_id",
        type="string",
        description="AMI ID. Pipe | char test.",
        is_sensitive=True,
    )  # Required

    out1 = TerraformOutputDoc(
        name="instance_ips", description="Public IPs.", is_sensitive=False
    )
    out2 = TerraformOutputDoc(
        name="app_url", description="Application URL.", is_sensitive=True
    )

    res1 = TerraformResourceDoc(
        resource_type="aws_instance", resource_name="web_app", source_file="main.tf"
    )
    res2 = TerraformResourceDoc(
        resource_type="aws_s3_bucket", resource_name="app_storage", source_file="s3.tf"
    )

    modcall1 = TerraformModuleCallDoc(
        module_name="networking", source="./modules/vpc", source_file="main.tf"
    )
    ...
    file_s3 = TerraformFileDoc(
        file_path="s3.tf", resources=[res2]
...
def test_markdown_renderer_module_header(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
...
def test_markdown_renderer_file_sections(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
...
def test_markdown_renderer_providers_section(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
...
def test_markdown_renderer_variables_section(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
    ...
    assert (
        "| `image_id` | AMI ID. Pipe \\| char test. | `string` | *(Required)* | `True` |"
        in md
    )
    assert (
        "| `instance_count` | Number of instances. | `number` | `1` | `False` |" in md
    )


def test_markdown_renderer_outputs_section(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
...
def test_markdown_renderer_resources_section(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
    ...
    s3_tf_section_end = (
        md.find("---", s3_tf_section_start)
        if "---" in md[s3_tf_section_start:]
        else len(md)
    )
...
def test_markdown_renderer_module_calls_section(
    sample_module_doc_data: TerraformModuleProcessedDoc,
):
    renderer = MarkdownRenderer(sample_module_doc_data)
    md = renderer.render_module_documentation()
...
def test_markdown_renderer_empty_sections_not_rendered():
    ...
    module_doc = TerraformModuleProcessedDoc(
        module_path="/empty/module", files=[file_empty]
    )

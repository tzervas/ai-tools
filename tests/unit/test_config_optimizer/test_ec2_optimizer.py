...
from src.mcp_tools.config_optimizer.aws.ec2_optimizer import check_ec2_instance_optimizations

# --- Test Data ---

def create_ec2_resource(id: str, name: str, instance_type: str, tags: Optional[Dict[str, str]] = None) -> ParsedResource:
...
def test_ec2_newer_generation_suggestion_generic_family():
    ...
    res_m4 = create_ec2_resource("i-m4", "test-m4", "m4.xlarge") # m4 is in map, m5/m6/m7 are newer
    rules_m4 = EC2InstanceTypeRule(suggest_newer_generations=True, generation_map={}) # Empty map to force generic
    ...
    assert any("m4.xlarge" in r.message and ("m5" in r.message or "m6" in r.message or "m7" in r.message) for r in recs_m4)

...
from src.mcp_tools.config_optimizer.aws.s3_optimizer import (
    check_s3_bucket_optimizations,
)
...
def create_s3_resource(
    id: str, name: str, attributes: Dict[str, Any]
) -> ParsedResource:
...
def test_s3_encryption_not_configured():
    ...
    rules = AWSS3Rules(
        encryption=S3BucketEncryptionRule(enabled=True, require_sse_kms=False)
    )
...
def test_s3_encryption_aes256_fail_when_kms_required():
    ...
    rules = AWSS3Rules(
        encryption=S3BucketEncryptionRule(enabled=True, require_sse_kms=True)
    )
    recs = check_s3_bucket_optimizations(res, rules)
    assert any(r.rule_id == "AWS_S3_ENCRYPTION_DISABLED" for r in recs)
...
def test_s3_versioning_enabled_api_style():  # API response: direct dict
    ...
    res_enabled_true = create_s3_resource(
        "bucket-ver-on-api2", "ver-on-api2", res_attrs_enabled_true
    )
...
def test_s3_pab_not_configured():
    ...
    rules = AWSS3Rules(
        public_access_block=S3BucketPublicAccessBlockRule(
            enabled=True, require_all_blocks_true=True
        )
    )
    recs = check_s3_bucket_optimizations(res, rules)
...
def test_s3_pab_partially_configured_false():  # TF style list of dicts
    ...
    recs = check_s3_bucket_optimizations(res, rules)
    assert any(r.rule_id == "AWS_S3_PUBLIC_ACCESS_BLOCK_INCOMPLETE" for r in recs)
    assert "block_public_policy" in recs[-1].details.get(
        "blocks_not_fully_enabled", {}
    )  # Assuming it's the last rec
...
def test_s3_all_aws_s3_rules_disabled():
    res = create_s3_resource(
        "bucket-all-s3-off", "all-s3-off", {}
    )  # Will fail all checks if enabled

...
def test_load_optimizer_rules_no_file_present():
    ...
    with pytest.MonkeyPatch.context() as mp:
        temp_dir = (
            Path(original_cwd) / "temp_test_dir_for_load_config_no_file"
        )  # Avoid using tmp_path directly here to control CWD
        temp_dir.mkdir(exist_ok=True)
        mp.chdir(temp_dir)
        try:
            config = load_optimizer_rules(
                config_path="non_existent_rules.yml"
            )  # Explicit non-existent
            assert (
                config.aws_ec2.instance_type_optimization.suggest_newer_generations
                is True
            )  # Default
...
def test_load_optimizer_rules_from_file(temp_optimizer_rules_file):
    custom_rules_data = {
        ...
        "aws_s3": {
            ...
            },  # This won't be checked due to parent
    ...
    assert config.aws_ec2.instance_type_optimization.large_instance_types_to_flag == [
        "m1.xlarge"
    ]
    assert config.aws_s3.enabled is False
    assert (
        config.aws_s3.encryption.require_sse_kms is True
    )  # Still parsed, just not used if aws_s3 disabled
    assert config.aws_s3.versioning.enabled is False
    assert (
        config.aws_s3.public_access_block.enabled is True
    )  # Default, as aws_s3.public_access_block was not in file
...
def test_load_optimizer_rules_partial_config(temp_optimizer_rules_file):
    partial_data = {
        "aws_ec2": {"instance_type_optimization": {"suggest_newer_generations": False}}
    ...
    assert (
        config.aws_s3.encryption.enabled is True
    )  # Default for S3BucketEncryptionRule
...
def test_default_file_search_logic_optimizer(tmp_path: Path):
    ...
    with open(
        project_root / DEFAULT_OPTIMIZER_RULES_FILENAME, "w"
    ) as f:  # Place in project_root
    ...
    try:
        config = (
            load_optimizer_rules()
        )  # No explicit path, should search up to project_root
        assert config.aws_s3.versioning.enabled is False
...
def test_load_optimizer_rules_explicit_path_not_found(capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "Warning: Optimizer rules file 'non_existent_rules.yml' not found. Using default rules."
        in captured.out
    )

...
from src.mcp_tools.pr_reviewer.config import (
    BranchNamingPolicy,
    CommitMessagePolicy, ConventionalCommitPolicy, RequireIssueNumberPolicy,
    DisallowedPatternsPolicy, DisallowedPatternItem,
    FileSizePolicy
)
...
def test_check_commit_for_issue_number(body, pattern_str, pr_title, pr_body, in_commit_body, expected_violations_count):
...
def test_check_commit_message_policies_orchestration():
    ...
    assert any("Type 'badtype' is not one of the allowed types" in v for v in violations)
...
@pytest.mark.parametrize("filepath, patterns_config, expected_violations_count, expected_messages_contain", [
    ...
    ("secrets.py", [DisallowedPatternItem(pattern="API_KEY", enabled=False)], 0, []), # Disabled pattern
    ("binary.data", [DisallowedPatternItem(pattern="SECRET_KEY", enabled=True)], 0, []), # Binary skipped
    ("utf8_error.txt", [DisallowedPatternItem(pattern="Invalid", enabled=True)], 0, []),# UTF-8 error skipped
])
def test_check_content_disallowed_patterns(filepath, patterns_config, expected_violations_count, expected_messages_contain):
...
def test_check_file_size_policy(filepath, max_bytes, ignore_ext, ignore_paths, expected_violations_count):

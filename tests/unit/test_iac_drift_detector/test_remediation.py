import pytest
from src.mcp_tools.iac_drift_detector.models import (
    DriftInfo,
    DriftType,
    AttributeDrift,
    ParsedResource,
...
def test_suggest_remediation_missing_in_actual_terraform():
    ...
    assert (
        f"Resource aws_instance.web_server_01 (Expected IaC ID: i-expected123) is defined in IaC but MISSING"
        in suggestions[0]
    )
...
def test_suggest_remediation_unmanaged_in_actual_terraform():
    ...
    assert (
        "Resource aws_s3_bucket.manual-bucket-007 (ID: manual-bucket-007-id) exists in the actual state but is UNMANAGED by IaC."
        in suggestions[0]
    )
    assert (
        "Suggestion 1: Import the resource into Terraform state using 'terraform import aws_s3_bucket.manual-bucket-007 manual-bucket-007-id'"
        in suggestions[2]
    )
...
def test_suggest_remediation_modified_terraform():
    ...
    drift = DriftInfo(
        ...
        actual_resource=ParsedResource(
            ...
            provider_name="aws",
            attributes={"instance_type": "t3.small", "monitoring": True},
        ),
    ...
    assert (
        "Resource aws_instance.app_server_main (ID: i-actual456) is MODIFIED."
        in suggestions[0]
    )
...
def test_suggest_remediation_unknown_drift_type():
    ...
    assert (
        "Unknown drift type 'very_unknown_drift' for resource some_type.some_name (ID: some_id)."
        in suggestions[0]
    )
...
def test_suggest_remediation_generic_iac_tool():
    ...
    suggestions = suggest_remediation(
        drift_missing, iac_tool="pulumi"
    )  # Example other tool
    assert len(suggestions) >= 2
    assert (
        "Use your IaC tool to apply the configuration and create the resource."
        in suggestions[1]
    )
    ...
    drift_unmanaged = DriftInfo(
        ...
        actual_resource=ParsedResource(
            id="unmanaged-id-456",
            type="another_resource",
            name="unmanaged_res",
            provider_name="any",
            attributes={},
        ),
    )
    suggestions_unmanaged = suggest_remediation(
        drift_unmanaged, iac_tool="cloudformation"
    )
    assert any(
        "Consider importing it into your IaC" in s for s in suggestions_unmanaged
    )
    ...
    drift_modified = DriftInfo(
        ...
        attribute_drifts=[
            AttributeDrift(attribute_name="color", iac_value="blue", actual_value="red")
        ],
    )
    suggestions_modified = suggest_remediation(drift_modified, iac_tool="ansible")
    assert any(
        "Review differences. Align actual state by applying IaC" in s
